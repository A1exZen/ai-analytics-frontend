...
import {createAnalysis, getAnalysisStatusViaSSE} from "@/api/analyzeApi.ts";
...
const Search: React.FC<SearchProps> = ({setError}) => {
			...
			const source = getAnalysisStatusViaSSE(taskId, (status: TaskStatus) => {
				if (status.status === "completed") {
					const newAnalysis: Analysis = {
						id: status.result?.id || response.id,
						url: query,
						data: status.result || ({} as AnalysisResponse),
					...
					navigate(`/analytics/${newAnalysis.id}`);
					stopLoading();
					...
					toast.error(status.error || "Произошла ошибка анализа.");
					if (eventSource) eventSource.close();
				}
			});
	...
	return (
		...
		>
			<motion.form className="relative" onSubmit={handleSearch}
			             variants={fadeIn}>
				...
				{isDropdownOpen && searchHistory.length > 0 && (
					...
					>
						<li
							className="flex justify-between items-center px-4 py-2 border-b border-gray-200/30 dark:border-gray-700/30">
